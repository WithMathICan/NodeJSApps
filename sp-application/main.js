...
const { findDbTables, createInterfaces, createPermissions } = require('../sp-core')
...
pgPool.query('SELECT 1+1').then(async () => {
   ...
   await fillModels(dbTables, PG_DATABASE, config.FK_TITLE_COLUMN)
   await fillControllers(dbTables, createDbClient)
   await createPermissions('secret.permission_value', 'secret.permission', dbTables, poolQuery)
